...
public class HandleTyreGrip : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        tiresOnGround = 0;
        if (GetComponent<RCC_CarControllerV3>().FrontRightWheelCollider.wheelCollider.isGrounded)
        {
            tiresOnGround++;
            airTime = 0f;
        }
        if (GetComponent<RCC_CarControllerV3>().FrontLeftWheelCollider.wheelCollider.isGrounded)
        {
            tiresOnGround++;
            airTime = 0f;
        }
        if (GetComponent<RCC_CarControllerV3>().RearRightWheelCollider.wheelCollider.isGrounded)
        {
            tiresOnGround++;
            airTime = 0f;
        }
        if (GetComponent<RCC_CarControllerV3>().RearLeftWheelCollider.wheelCollider.isGrounded)
        {
            tiresOnGround++;
            airTime = 0f;
        }
    ...
    {
        //track velocity//
        xVel = transform.InverseTransformDirection(GetComponent<Rigidbody>().linearVelocity).x;
        zVel = transform.InverseTransformDirection(GetComponent<Rigidbody>().linearVelocity).z;
