...
public class RCC_WheelCollider : MonoBehaviour {
	...
	public int groundIndex = 0;		// Current ground physic material index.
	...
	private float steeringSmoother{
		get{
			return carController.steeringSensitivity;
		}
	}
...
//		wheelRPMToSpeed = carController.speed;
	...
	void ApplyMotorTorque(float torque){
		...
		wheelCollider.motorTorque = (((torque * (1 - carController.clutchInput) * carController._gasInput * (1f + carController._boostInput)) * (carController.engineTorqueCurve[carController.currentGear].Evaluate(wheelRPMToSpeed * carController.direction) * carController.direction)) * carController.finalRatio);

	}

	public void ApplySteering(){

		wheelCollider.steerAngle = Mathf.Lerp(wheelCollider.steerAngle, Mathf.Clamp(carController.steerAngle * carController._steerInput, -carController.steerAngle, carController.steerAngle) * steeringMultiplier, steeringSmoother * Time.fixedDeltaTime);
	...
	int GetGroundMaterialIndex(){
		...
		if(!contacted){
			...
			for (int i = 0; i < RCCGroundMaterials.terrainFrictions.Length; i++) {
				...
				if (GroundHit.collider.sharedMaterial == RCCGroundMaterials.terrainFrictions [i].groundMaterial) {
