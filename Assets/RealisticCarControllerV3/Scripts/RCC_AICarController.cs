...
public class RCC_AICarController : MonoBehaviour
{
	...
	void Update()
	{
		...
		{
			//Debug.Log(targetsInZone[i]);
			if (targetsInZone[i] == null)
				targetsInZone.RemoveAt(i);
			
				else if (!targetsInZone[i].gameObject.activeInHierarchy)
					targetsInZone.RemoveAt(i);
	...
	void OnTriggerEnter(Collider col)
	{
		...
		if (col.attachedRigidbody != null && col.transform.root && col.transform.root.CompareTag(targetTag))
		{
