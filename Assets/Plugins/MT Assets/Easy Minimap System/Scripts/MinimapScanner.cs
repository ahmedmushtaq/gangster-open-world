    ...
    */
    ...
    public class MinimapScanner : MonoBehaviour
    {
        ...
        public void DoScanInThisAreaOfComponentAndShowOnMinimap()
        {
            ...
            //Stop image processing, if enabled
            if (imageProcessing == ImageProcessing.Enabled)
                StopImageProcessing();
            ...
            if (Application.isPlaying == false)
            {
                ...
                if (!AssetDatabase.IsValidFolder("Assets/Plugins/MT Assets/_AssetsData/Scans"))
                    AssetDatabase.CreateFolder("Assets/Plugins/MT Assets/_AssetsData", "Scans");

                //Generate a name for the scan file
                DateTime date = DateTime.Now;
                pathForFile = "Assets/Plugins/MT Assets/_AssetsData/Scans/Scan (" + date.Year + date.Month + date.Day + date.Hour + date.Minute + date.Second + date.Millisecond.ToString() + ").png";
